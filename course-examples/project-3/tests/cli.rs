...
fn cli_log_configuration() {
    let temp_dir = TempDir::new().unwrap();
    let stderr_path: PathBuf = temp_dir.path().join("stderr");
    let mut cmd = Command::cargo_bin("kvs-server").unwrap();
    let mut child = cmd
    ...
    assert!(Path::new(stderr_path.as_os_str()).is_file());
    let temp_dir_content = fs::read_to_string(stderr_path);
    println!("DEBUG: temp_dir content: {:?}", temp_dir_content);
